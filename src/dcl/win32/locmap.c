 ...
 * Modified by Dennis Prochko <wolfsoft@mail.ru>
 */
...
#define U_USING_FALLBACK_WARNING	1
#define U_ILLEGAL_ARGUMENT_ERROR	2
#define U_ZERO_ERROR				3
 ...
 */
static uint32_t
getHostID(const ILcidPosixMap *this_0, const char* posixID, uint32_t* status)
{
    int32_t bestIdx = 0;
...
#endif
const char *
convertToPosix(uint32_t hostid, uint32_t* status)
{
    uint16_t langID;
    ...
    memset(gPosixFromLCID, 0, sizeof(gPosixFromLCID));
...
uint32_t
convertToLCID(const char *langID, const char* posixID, uint32_t* status)
{
